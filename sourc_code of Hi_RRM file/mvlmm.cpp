...
void tmat( gsl_matrix *U_l,gsl_vector *h_W){
    size_t d_size = U_l->size1;
    gsl_matrix *Ul_temp = gsl_matrix_alloc(d_size, d_size);
    gsl_vector *h_W_temp =gsl_vector_alloc(d_size);
     for (size_t i = 0; i < d_size; i++) {
        for (size_t j = 0; j < d_size; j++) {
          gsl_matrix_set(Ul_temp, i, j, gsl_matrix_get(U_l, i, d_size-j-1));
        }
        gsl_vector_set(h_W_temp, i, gsl_vector_get(h_W, d_size - i - 1));
      }
      gsl_matrix_memcpy(U_l, Ul_temp);
      gsl_vector_memcpy(h_W, h_W_temp);
}

void calQ(const gsl_matrix *V_g, const gsl_matrix *V_e, gsl_vector *h_W,gsl_matrix *Q) {
  size_t d_size = V_g->size1;
  double d;
  // Eigen decomposition of V_e.
  gsl_matrix *Lambda = gsl_matrix_alloc(d_size, d_size);
  gsl_matrix *Ul_temp = gsl_matrix_alloc(d_size, d_size);
   gsl_matrix *V_e_temp = gsl_matrix_alloc(d_size, d_size);
  gsl_vector *h_W_temp =gsl_vector_alloc(d_size);
  gsl_matrix *P = gsl_matrix_alloc(d_size, d_size);
  gsl_matrix *V_e_hi = gsl_matrix_alloc(d_size, d_size);
  gsl_matrix *VgVehi = gsl_matrix_alloc(d_size, d_size);
  gsl_matrix *U_l = gsl_matrix_alloc(d_size, d_size);

  gsl_matrix_memcpy(V_e_temp, V_e);
  EigenDecomp(V_e_temp, U_l, h_W, 0);
  tmat(U_l, h_W);
  // Calculate V_e_h and V_e_hi.
  gsl_matrix_set_zero(P);
  gsl_matrix_set_zero(V_e_hi);
  for (size_t i = 0; i < d_size; i++) {
    d = gsl_vector_get(h_W, i);
    if (d <= 0) {
      continue;
    }
    d = safe_sqrt(d);
    d = 1.0 / d;
    for (size_t j = 0; j < d_size; j++) {
      gsl_matrix_set(V_e_hi, j, i, gsl_matrix_get(U_l, j, i)*d);
    }
  }
  gsl_blas_dgemm(CblasNoTrans, CblasTrans, 1.0, V_e_hi,U_l , 0.0, P);
  // Calculate Lambda=V_ehi V_g V_ehi.
  gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1.0, P,V_g , 0.0, VgVehi);
  gsl_blas_dgemm(CblasNoTrans, CblasTrans, 1.0, VgVehi, P, 0.0, Lambda);
  // Eigen decomposition of Lambda.
  EigenDecomp(Lambda, U_l, h_W, 0);
  tmat(U_l, h_W);
  for (size_t i = 0; i < d_size; i++) {
    d = gsl_vector_get(h_W, i);
    if (d < 0) {
      gsl_vector_set(h_W, i, 0);
    }
  }
  gsl_blas_dgemm(CblasTrans, CblasNoTrans, 1.0,U_l , P, 0.0, Q);
  // free memory
  gsl_matrix_free(Lambda);
  gsl_matrix_free(V_e_temp);
  gsl_matrix_free(Ul_temp);
  gsl_matrix_free(V_e_hi);
  gsl_matrix_free(VgVehi);
  gsl_matrix_free(U_l);
  return ;
...
void GEMMAXregression(const gsl_matrix *X,const gsl_vector *Y,float coef,float vb){
    size_t n_size = Y->size, r_size = X->size2;
    gsl_matrix *xPx = gsl_matrix_alloc(r_size, r_size);
    gsl_matrix *Vbeta = gsl_matrix_alloc(r_size, r_size);
    gsl_vector *xPy = gsl_vector_alloc(r_size);
    gsl_vector *tempbeta = gsl_vector_alloc(r_size);
    gsl_blas_dgemm(CblasTrans, CblasNoTrans, 1.0, X, X, 0.0, xPx);
    gsl_blas_dgemv(CblasTrans,  1.0, X, Y, 0.0, xPy);
    int sig;
    gsl_permutation *pmt = gsl_permutation_alloc(r_size);
    LUDecomp(xPx, pmt, &sig);
    LUSolve(xPx, pmt, xPy, tempbeta);
    LUInvert(xPx, pmt, Vbeta);
    coef = gsl_vector_get(tempbeta,r_size-1); 
    vb = gsl_matrix_get(Vbeta,r_size-1,r_size-1);
    gsl_vector_free(xPy);
    gsl_vector_free(tempbeta);
    gsl_matrix_free(xPx);
    gsl_matrix_free(Vbeta);
    return;
}
//p_score = GEMMAXCalcP(eval, &X_row.vector, &X_sub.matrix, Y, position,
   //                          logV0, V0, beta, Vbeta);
double GEMMAXCalcP(const gsl_vector *eval, const gsl_vector *x_vec,const gsl_matrix *W,const gsl_matrix *Y,
                const gsl_vector *position,const gsl_vector *logV0,const gsl_matrix *V050,const float p_nr,
                gsl_vector *beta,gsl_matrix *Vbeta) {
  size_t n_size = eval->size, c_size = W->size1, d_size = position->size;
  size_t r_size = 1 + c_size;
  double coef,vb; //  logdet_Ve, logdet_Q, p_value;
  gsl_matrix *Xregression = gsl_matrix_alloc(n_size, r_size);
  gsl_matrix *WXregression = gsl_matrix_alloc(n_size, r_size);
  gsl_multifit_linear_workspace * work = gsl_multifit_linear_alloc (n_size, r_size);
  gsl_vector *sY = gsl_vector_alloc(n_size);
  gsl_vector *temp = gsl_vector_alloc(n_size);
  gsl_vector *tempV0 = gsl_vector_alloc(n_size);
  gsl_vector *tempbeta = gsl_vector_alloc(r_size);
  gsl_matrix *tempH = gsl_matrix_alloc(r_size, r_size);
  gsl_matrix_set_col(Xregression,c_size,x_vec);
  double chi,chiall;
  size_t rank;
  for (int i = 0; i < c_size; ++i)
  {
    gsl_matrix_get_row(temp,W,i);
    gsl_matrix_set_col(Xregression,i,temp);
  }
  chiall=0;  
  for (int j = 0; j < d_size; ++j)
  {
      int pos = gsl_vector_get(position,j);
      gsl_matrix_get_col(tempV0,V050,pos);
      gsl_matrix_get_col(sY,Y,j); 
      gsl_vector_mul(sY,tempV0);
      for (int i = 0; i < r_size; ++i)
      {
          gsl_matrix_get_col(temp,Xregression,i);
          gsl_vector_mul(temp,tempV0);
          gsl_matrix_set_col(WXregression,i,temp);
      }
      //GEMMAXregression(WXregression,sY,coef,vb);
      //gsl_vector_set(beta,j,coef);
      //gsl_matrix_set(Vbeta,j,j,vb);
      gsl_multifit_linear_tsvd(WXregression,sY,0.001,tempbeta,tempH,&chi,&rank,work);
      gsl_vector_set(beta,j,gsl_vector_get(tempbeta,c_size));
      gsl_matrix_set(Vbeta,j,j,gsl_matrix_get(tempH,c_size,c_size));
      chiall += gsl_vector_get(tempbeta,c_size)*gsl_vector_get(tempbeta,c_size)/gsl_matrix_get(tempH,c_size,c_size);
      //chiall += coef*coef/vb;  
  }

  double p_value = gsl_cdf_chisq_Q(chiall, (double)d_size);
  float ve,loglik,preloglik;
  if(p_value < p_nr ){
     // gsl_multifit_linear_workspace * work = gsl_multifit_linear_alloc (n_size, r_size);
      chiall = 0;
      gsl_vector *resi = gsl_vector_alloc(n_size);
      for (int j = 0; j < d_size; ++j)
       {
        int pos = gsl_vector_get(position,j);
        preloglik = gsl_vector_get(logV0,pos);
        for (int ii = pos ;ii > 0;)
        {
            int step = 1;
            gsl_matrix_get_col(tempV0,V050,ii);
            gsl_matrix_get_col(sY,Y,j); 
            gsl_vector_mul(sY,tempV0);
            for (int i = 0; i < r_size; ++i)
            {
            ...
            gsl_multifit_linear_residuals(WXregression,sY,tempbeta,resi);
            ve = gsl_stats_variance(resi->data,1,n_size);
            loglik = n_size * log(ve)+ gsl_vector_get(logV0,ii);       
            if(loglik > preloglik){
                  gsl_multifit_linear_tsvd(WXregression,sY,0.001,tempbeta,tempH,&chi,&rank,work);
                  gsl_vector_set(beta,j,gsl_vector_get(tempbeta,c_size));
                  gsl_matrix_set(Vbeta,j,j,gsl_matrix_get(tempH,c_size,c_size));
      ...
      p_value = gsl_cdf_chisq_Q(chiall, (double)d_size);
  }
  gsl_matrix_free(Xregression);
  gsl_matrix_free(WXregression);
  gsl_matrix_free(tempH);
  gsl_vector_free(sY);
...
int h2estimate(gsl_vector *ynew,gsl_matrix *X,
      gsl_matrix *V050,gsl_vector * logV0)
{
    int n_size = ynew -> size; 
    int r_size = X -> size2;
    float pre_loglike = 9999999999; 
    float ve,vb,Log;  
    gsl_matrix *WXregression = gsl_matrix_alloc(n_size, r_size);
    gsl_vector *sY = gsl_vector_alloc(n_size);
    gsl_vector *temp = gsl_vector_alloc(n_size);
    ...
    {
        gsl_matrix_get_col(tempV0,V050,i);
        gsl_vector_memcpy(sY,ynew);
        gsl_vector_mul(sY,tempV0);
        for (int i = 0; i < r_size; ++i)
        {
          gsl_matrix_get_col(temp,X,i);
          gsl_vector_mul(temp,tempV0);
          gsl_matrix_set_col(WXregression,i,temp);
        }
        gsl_multifit_linear_residuals(WXregression,sY,tempbeta,resi);
        ve = gsl_stats_variance(resi->data,1,n_size);
        Log = n_size * log(ve)+ gsl_vector_get(logV0,i); 
        if(Log > pre_loglike){ 
          return i;
          }
        pre_loglike = Log;
...
int FindMin(gsl_vector *a, int n, int *pMinPos)
{
    int i; 
    float min;
    min = gsl_vector_get(a,0);              
    *pMinPos = 0;           
    for (i=1; i<n; i++)
    {
        if (gsl_vector_get(a,i) < min)
        {
            min = gsl_vector_get(a,i);
            *pMinPos = i;  
        }
    }
    return min ;
...
//resultsize = fast_jiont(phenonew,&X_sub.matrix,JiontselectX,V050,logV0,Jiontselectposi,result);
int fast_jiont(gsl_matrix *ynew,gsl_matrix *gnewb0,gsl_matrix *gnew,
                      gsl_matrix *V050,gsl_vector *logV0,
                      gsl_vector *position,gsl_matrix *result) {
   int nobs = ynew->size1;
   int nt = ynew->size2;
   int c_size = gnewb0->size1;
   double minValue,chi,tempchi;
   size_t nmar = gnew->size2;
   int r_size = nmar+c_size;
   gsl_matrix *Gall = gsl_matrix_alloc(nobs, r_size);
   gsl_matrix *WGall = gsl_matrix_alloc(nobs, r_size);
   gsl_matrix *gnew1 = gsl_matrix_alloc(nobs, r_size);
   gsl_vector *ynew1 = gsl_vector_alloc(nobs);
   gsl_vector *temp = gsl_vector_alloc(nobs);
   gsl_vector *tempV0 = gsl_vector_alloc(nobs);
   gsl_vector *chiall = gsl_vector_alloc(nmar);
   gsl_vector *Ys = gsl_vector_alloc(nobs);
   ...
   for (int i = 0; i < c_size; ++i)
   {
     gsl_matrix_get_row(temp,gnewb0,i);
     gsl_matrix_set_col(Gall,i,temp);
   }
  
   for (int i = c_size; i < r_size; ++i)
   {
      gsl_matrix_get_col(temp,gnew,i-c_size);
      gsl_matrix_set_col(Gall,i,temp);
   }
   
   int h2pos; 
   gsl_vector_set_zero(chiall);
   size_t rank;
   gsl_multifit_linear_workspace * work = gsl_multifit_linear_alloc (nobs, r_size);
   for(int j=0;j<nt;j++){
      gsl_matrix_get_col(Ys,ynew,j);
      h2pos = h2estimate(Ys,Gall,V050,logV0);
      gsl_matrix_get_col(tempV0,V050,h2pos);
      gsl_vector_mul(Ys,tempV0);

      for (int i = 0; i < r_size; ++i)
            {
              gsl_matrix_get_col(temp,Gall,i);
              gsl_vector_mul(temp,tempV0);
              gsl_matrix_set_col(WGall,i,temp);
      }     
      gsl_multifit_linear_tsvd(WGall,Ys,0.001,tempbeta,tempH,&chi,&rank,work); 
      for (int i = c_size; i < r_size; ++i)
      {
        tempchi=gsl_vector_get(tempbeta,i)*gsl_vector_get(tempbeta,i)/gsl_matrix_get(tempH,i,i)+gsl_vector_get(chiall,i-c_size);
        if(std::isnan(tempchi)){tempchi=0.00001;}
        //cout<<tempchi<<endl;
        gsl_vector_set(chiall,i-c_size,tempchi);
      }
   }
   ...
   for(size_t i=0; i<nmar; ++i) { 
        //minPos = gsl_vector_min_index(chiall);     
        //minValue =  gsl_vector_min(chiall); 
        minValue = FindMin(chiall, nmar-i-1, &minPos); 
        minValue = gsl_vector_get(chiall,minPos);
        //cout<<minPos<<endl; 
        //cout<<minValue<<endl;
        if(minValue < 0.000001 ){break;}
        if(gsl_cdf_chisq_Q(minValue, (double)nt) < 0.01){
            int finallength = chiall->size;
            for (int i = 0; i < finallength; ++i)
              {
              gsl_matrix_set(result,i,0,gsl_vector_get(position,i));
              gsl_matrix_set(result,i,1,gsl_vector_get(chiall,i));
             }
            return finallength;
        }    
        
        gnew1=gsl_matrix_alloc(nobs,r_size-i-1);
        gsl_matrix_set_zero(gnew1);
        chiall = gsl_vector_alloc(nmar-i-1);
        gsl_vector_set_zero(chiall);
        gsl_vector *position_changable = gsl_vector_alloc(nmar-i-1);
        gsl_vector_set_zero(position_changable);
        gsl_matrix *Gall_changable = gsl_matrix_alloc(nobs,r_size-i-1);
        gsl_matrix_set_zero(Gall_changable);
        
        for (int ii = 0; ii < c_size; ++ii)
         {
           gsl_matrix_get_row(temp,gnewb0,ii);
           gsl_matrix_set_col(Gall_changable,ii,temp);
         }      
        int count = 0;
        for (int ii = 0; ii < nmar-i-1; ) {
          if(count != minPos) {
            gsl_vector_set(position_changable,ii,gsl_vector_get(position,count));
            gsl_matrix_get_col(temp,Gall,ii+c_size);
            gsl_matrix_set_col(Gall_changable,ii+c_size,temp);
            ++ii;
          }
          count++;
        }          
        position = gsl_vector_alloc(nmar-i-1);
        gsl_vector_set_zero(position);
        Gall=gsl_matrix_alloc(nobs,r_size-i-1);
        gsl_matrix_set_zero(Gall);
        WGall=gsl_matrix_alloc(nobs,r_size-i-1);
        gsl_matrix_set_zero(WGall);
        gsl_matrix_memcpy(Gall,Gall_changable);
        gsl_vector_memcpy(position,position_changable);
        tempbeta = gsl_vector_alloc(r_size-i-1);
        gsl_vector_set_zero(tempbeta);
        tempH=gsl_matrix_alloc(r_size-i-1,r_size-i-1);
        gsl_matrix_set_zero(tempH);
     
        gsl_multifit_linear_workspace * work = gsl_multifit_linear_alloc (nobs, r_size-i-1);
        ...
        for(int j=0;j<nt;j++){   
          ...
          gsl_matrix_get_col(tempV0,V050,h2pos);
          gsl_vector_mul(Ys,tempV0);           
            for (int ij = 0; ij < r_size-i-1; ++ij)
                  {
                    gsl_matrix_get_col(temp,Gall,ij);
                    gsl_vector_mul(temp,tempV0);
                    gsl_matrix_set_col(WGall,ij,temp);
            }          
          gsl_multifit_linear_tsvd(WGall,Ys,0.001,tempbeta,tempH,&chi,&rank,work);
          for (int ij = c_size; ij < r_size-i-1; ++ij)
          {
            tempchi=gsl_vector_get(tempbeta,ij)*gsl_vector_get(tempbeta,ij)/gsl_matrix_get(tempH,ij,ij)+gsl_vector_get(chiall,ij-c_size);
            if(std::isnan(tempchi)){tempchi=0.00001;}
            //cout<<tempchi<<endl;
            gsl_vector_set(chiall,ij-c_size,tempchi);
          }   
     }
      gsl_multifit_linear_free(work);
    } 
...
void MVLMM::AnalyzePlink(const gsl_matrix *U, const gsl_vector *eval,
  ...
  gsl_matrix *UltVehiU = gsl_matrix_alloc(d_size, n_size);
  gsl_matrix *UltVehiE = gsl_matrix_alloc(d_size, n_size);
  gsl_matrix *Q = gsl_matrix_alloc(d_size, d_size);
 
  gsl_matrix *JiontX = gsl_matrix_alloc(n_size, 10000);
  gsl_vector *Jiontposi = gsl_vector_alloc(10000);
  size_t Jiontnumber = 0;
  

  gsl_vector *h_W = gsl_vector_alloc(d_size);
  //gsl_vector *hh = gsl_vector_alloc(1000);
  gsl_matrix *V050 = gsl_matrix_alloc(n_size, 1000);
  gsl_vector *logV0 = gsl_vector_alloc(1000);
  float SumlogV0,hh;
  for (int i = 0; i < 1000; ++i)
  {
    hh = i*0.001/(1-i*0.001);
    SumlogV0=0;
      for (int j = 0; j < n_size; ++j)
      {
          float v0 = gsl_vector_get(eval,j)*hh +1;
          SumlogV0 += log(v0);
          gsl_matrix_set(V050, j, i, 1/safe_sqrt(v0));
      }
      gsl_vector_set(logV0,i,SumlogV0);
  }
  // Large matrices for NR.
  // Each dxd block is H_k^{-1}.
  gsl_matrix *Hi_all = gsl_matrix_alloc(d_size, d_size * n_size);
  ...
  gsl_matrix *Y = gsl_matrix_alloc(d_size, n_size);
  gsl_matrix *phenonew = gsl_matrix_alloc(n_size,d_size);
  gsl_matrix *X = gsl_matrix_alloc(c_size + 1, n_size);
  gsl_matrix *V_g = gsl_matrix_alloc(d_size, d_size);
  gsl_matrix *V_e = gsl_matrix_alloc(d_size, d_size);
  gsl_matrix *B = gsl_matrix_alloc(d_size, c_size + 1);
  gsl_vector *beta = gsl_vector_alloc(d_size);
  gsl_matrix *Vbeta = gsl_matrix_alloc(d_size, d_size);
  gsl_vector *position = gsl_vector_alloc(d_size);
  // Null estimates for initial values.
  gsl_matrix *V_g_null = gsl_matrix_alloc(d_size, d_size);
  gsl_matrix *V_e_null = gsl_matrix_alloc(d_size, d_size);
  gsl_matrix *B_null = gsl_matrix_alloc(d_size, c_size + 1);
  ...
  for (size_t i = 0; i < d_size; i++) {
    for (size_t j = 0; j <= i; j++) {
      cout << gsl_matrix_get(V_g, i, j) << "\t";
      writefile  << gsl_matrix_get(V_g, i, j)<< "\n";
    }
    cout << endl;
  }
  cout << "se(Vg): " << endl;
  ...
  for (size_t i = 0; i < d_size; i++) {
    for (size_t j = 0; j <= i; j++) {
      cout << gsl_matrix_get(V_e, i, j) << "\t";
      writefile  << gsl_matrix_get(V_e, i, j)<< "\n";
    }
    cout << endl;
  }
  cout << "se(Ve): " << endl;
