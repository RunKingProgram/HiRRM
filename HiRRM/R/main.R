...
Hi_RRM<-function(plinkfilename,y){
  ...
  Vg<-matrix(,nt,nt)
  Ve<-matrix(,nt,nt)
  fwrite(y,"Regs.phenotype",col.names=F,row.names=F,quote=F)
  ...
  system(paste("./Hi_RRM -bfile ",plinkfilename,
               " -p Regs.phenotype -k ./output/kinship.sXX.txt -lmm 3 -n ",ntraits," -o vc",sep=""))
  ...
  colnames(result)=c(paste("Beta",1:nt,sep=" "),paste("Vb",1:nt,sep=" "),"Chisq","P-value")
  bim=fread(paste(plinkfilename,".bim",sep=""))
  result=cbind(bim[,c(1,4,2)],result)
  colnames(result)[1:3]=c("Chr","Pos","SNP")
  fwrite(result,"allasso.txt",col.names=T,row.names=F,quote=F)
  fwrite(result[which(pv<(0.05/nmar)),],"sigasso.txt",col.names=T,row.names=F,quote=F)
